...
reg = lambda/(2*m) * ( sum(sum(Theta_1.^2)) + sum(sum(Theta_2.^2)) );
J = (1/m) * sum(sum(-yVectors .* log(h) - (1-yVectors) .* log(1-h))) + reg;

for t = 1:m
    a_1 = [1 X(t, :)]; % 1x401
    z_2 = a_1 * Theta1'; % 1x401 * 401x25 = 1*25
    a_2 = sigmoid(z_2);
    a_2 = [1 a_2]; % add bias node 1x26
    z_3 = a_2 * Theta2'; % 1x26 * 26x10 = 1x10
    a_3 = sigmoid(z_3);

    delta_3 = a_3 - yVectors(t, :); % 1x10 - 1x10 = 1x10
    delta_2 = delta_3 * Theta2 .* sigmoidGradient([1 z_2]); % 1x10 * 10x26 = 1x26
